...
public class TrapItem {
    ...
    public void render(SpriteBatch batch) {
        ...
        for (int i = 0; i < trapItemPositions.length; i++) {
            if (!isTrapItemCollected[i]) {
                batch.draw(trapItems[i],
                        trapItemPositions[i].x,
                        trapItemPositions[i].y,
                        trapItems[i].getWidth() * scale,
                        trapItems[i].getHeight() * scale);
            }
        }
    }
    ...
    public boolean checkAndCollectTrapItem(Vector2 playerPosition, float proximity, int index) {
        if (!isTrapItemCollected[index]) {
            ...
            if (distance <= proximity) {
                ...
                trapItemPositions[index] = new Vector2(-1000, -1000);
    ...
    private void activateFogEffect() {
        isFogActive = true;
